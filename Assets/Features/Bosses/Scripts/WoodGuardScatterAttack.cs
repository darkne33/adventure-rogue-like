        ...
        public async UniTask Execute(CancellationToken cancellationToken, bool animateBoss = true)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    {
                        while (positions.Count < spawnCount && elapsed >= nextSpawnTime)
                        {
                            ...
                            if (!animationStarted)
                            {
                                ...
                                firstWarningTime = elapsed;
                                if (animateBoss)
                                {
                                    _boss.AnimationSystem.BeginAttack(_configuration.SafeWarningDuration);
                                    _boss.AnimationSystem.SetTimeScale(timeScale);
                                }
                            }
                            ...
                            pieces.Add(_singleAttack.ExecuteAt(position, false,
                                _configuration.ShowWarning, sequenceToken));
                        ...
                        if (positions.Count == spawnCount && !animationFinished &&
                            elapsed - firstWarningTime >= _configuration.SafeWarningDuration)
                        {
